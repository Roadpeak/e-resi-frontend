      ...
      <div className="max-w-screen-xl mx-auto px-8 sm:px-14 lg:px-20 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 lg:gap-8 mb-20">
          ...
          <div className="lg:col-span-1">
            <div className="mb-4">
              <Logo markSize={30} onDark textClassName="text-chalk text-2xl" />
            </div>
            <p className="text-stone/45 text-sm leading-relaxed max-w-xs">
          ...
          {cols.map((col) => (
            <div key={col.heading}>
              <p className="text-[10px] tracking-[0.2em] uppercase text-stone/35 mb-5">{col.heading}</p>
              <ul className="space-y-3">
                {col.links.map((l) => (
                  <li key={l.label}>
                    <Link
                      href={l.href}
                      className="text-stone/50 text-sm hover:text-chalk transition-colors duration-300"
                    >
                      {l.label}
                    </Link>
                  </li>
        ...
        <div className="border-t border-white/[0.05] pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-stone/25 text-xs tracking-wide">
            © {new Date().getFullYear()} e-resi. All rights reserved.
          </p>
          <p className="text-stone/20 text-xs tracking-wide">
            Nairobi, Kenya
          </p>
        </div>
      </div>
